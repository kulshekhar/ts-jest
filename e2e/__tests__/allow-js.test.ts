...
describe('using babel-jest for js files', () => {
  const outDirTestCase = configureTestCase('allow-js', {
    jestConfig: { testMatch: null, testRegex: '(foo|bar)\\.spec\\.[jt]s$' },
  })

  outDirTestCase.runWithTemplates(allValidPackageSets, 0, (runTest, { testLabel }) => {
    it(testLabel, () => {
...
describe('using ts-jest for js files', () => {
  ...
  outDirTestCase.runWithTemplates(allPackageSetsWithPreset, 0, (runTest, { testLabel }) => {
    it(testLabel, () => {
      const result = runTest()
      expect(result.status).toBe(0)
      expect(result).toMatchSnapshot()
