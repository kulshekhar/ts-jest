...
const DIR = 'const-enum'

test('successfully runs the tests inside `const-enum/` with `isolatedModules: false`', () => {
  const { json } = runWithJson(DIR)

  expect(json.success).toBe(true)
})

test('partial successfully runs the tests inside `const-enum/` with `isolatedModules: true`', () => {
